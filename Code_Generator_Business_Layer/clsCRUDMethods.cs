...
namespace Code_Generator_Business_Layer
{
    public class clsCRUDMethods
    {
        ...
        //Update Methods
        private static string GetIDentityParametersName(List<strColumnsInfo> columnsInfos)
        {
            List<string> IdentityColumnsName = new List<string>();
            foreach(strColumnsInfo columnInfo in columnsInfos)
            {
                if(columnInfo.IsIdentity)
                    IdentityColumnsName.Add(columnInfo.ColumnName);
            }
            
            return string.Join(",", IdentityColumnsName);
        }
